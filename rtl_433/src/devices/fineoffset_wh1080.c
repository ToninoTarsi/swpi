 ...
 * 
 *
 * 2016 Nicola Quiriti (aka 'ovrheat', 'seven')
 *
 *
 */
...
//--------- SWPI: (over)writing data to file '/dev/shm/wh1080-rtl_433.txt' --------------
	FILE *fp;
	fp=fopen("/dev/shm/wh1080-rtl_433.txt", "w");
	if (msg_type == 0) {
		fprintf(fp,"{\"time\" : \"%s\", ", time_str);
		fprintf(fp,"\"msg_type\" : %01X, ", msg_type);
		fprintf(fp,"\"station_id\" : \"%04X\", ", device_id);
		fprintf(fp,"\"temperature\" : %0.1f, ", temperature);
		fprintf(fp,"\"humidity\" : %d, ", humidity);
		fprintf(fp,"\"wind_speed\" : %0.2f, ", speed);
		fprintf(fp,"\"wind_gust\" : %0.2f, ", gust);
		fprintf(fp,"\"wind_dir_str\" : \"%s\", ", direction_str);
		fprintf(fp,"\"wind_dir_deg\" : \"%s\", ", direction_deg);
		fprintf(fp,"\"total_rain\" : %0.1f}", rain);
	} else {
		fprintf(fp,"{\"time\" : \"%s\", ", time_str);
		fprintf(fp,"\"msg_type\" : %01X, ", msg_type);
		fprintf(fp,"\"station_id\" : \"%04X\", ", device_id);
		fprintf(fp,"\"hours\" : \"%02d\", ", the_hours);
		fprintf(fp,"\"minutes\" : \"%02d\", ", the_minutes);
		fprintf(fp,"\"seconds\" : \"%02d\", ", the_seconds);
		fprintf(fp,"\"year\" : \"%02d\", ", the_year);
		fprintf(fp,"\"month\" : \"%02d\", ", the_month);
		fprintf(fp,"\"day\" : \"%02d\"}", the_day);		
		}	
	fclose(fp);	
