 ...
 * UV/Light radio messages are disjointed from (and shorter than) the weather/datetime radio messages and are transmitted 
 ...
 * 
 * While sometime the LCD console seems to truncate/round decimals to best fit to the display, this module never truncate 
 * them (but it will correctly rounds if necessary), so you can see -sometimes- some little difference with console's values.
 *
 *
...
static int fineoffset_wh1080_wh3080_callback(bitbuffer_t *bitbuffer) {
    data_t *data;
    char time_str[LOCAL_TIME_BUFLEN];
    const uint8_t *br;
    int msg_type = 0; // 0=Weather 1=Datetime 2=UV/Light
    int sens_msg = 0; // 12=Weather/Time sensor  8=UV/Light sensor
    int i;
	...
	sens_msg = 12;
        br = bitbuffer->bb[0];
    } else if(bitbuffer->bits_per_row[0] == 87) {
	sens_msg = 12;
        ...
        br = bbuf;
        bbuf[0] = 0xFF;
    } else if(bitbuffer->bits_per_row[0] == 64) {
	sens_msg = 8;
        br = bitbuffer->bb[0];

    } else if(bitbuffer->bits_per_row[0] == 63) {
	sens_msg = 8;
        /* 7 bits of preamble, bit shift the whole buffer and fix the bytestream */
        bitbuffer_extract_bytes(bitbuffer, 0, 7,
        (uint8_t *) & bbuf +1, 7*8);
...
//--------- SWPI: (over)writing data to files '/dev/shm/wh1080-rtl_433.txt'
//--------- and '/dev/shm/wh3080-rtl_433.txt'

if ((msg_type == 0) || (msg_type == 1)) {
	FILE *fpWH1080;
	fpWH1080=fopen("/dev/shm/wh1080-rtl_433.txt", "w");
	if (msg_type == 0) {
		fprintf(fpWH1080,"{\"time\" : \"%s\", ", time_str);
		fprintf(fpWH1080,"\"msg_type\" : %01X, ", msg_type);
		fprintf(fpWH1080,"\"station_id\" : \"%d\", ", device_id);
		fprintf(fpWH1080,"\"temperature\" : %0.1f, ", temperature);
		fprintf(fpWH1080,"\"humidity\" : %d, ", humidity);
		fprintf(fpWH1080,"\"wind_speed\" : %0.2f, ", speed);
		fprintf(fpWH1080,"\"wind_gust\" : %0.2f, ", gust);
		fprintf(fpWH1080,"\"wind_dir_str\" : \"%s\", ", direction_str);
		fprintf(fpWH1080,"\"wind_dir_deg\" : \"%s\", ", direction_deg);
		fprintf(fpWH1080,"\"total_rain\" : %0.1f}", rain);
	} else {
		fprintf(fpWH1080,"{\"time\" : \"%s\", ", time_str);
		fprintf(fpWH1080,"\"msg_type\" : %01X, ", msg_type);
		fprintf(fpWH1080,"\"station_id\" : \"%d\", ", device_id);
		fprintf(fpWH1080,"\"hours\" : \"%02d\", ", hours);
		fprintf(fpWH1080,"\"minutes\" : \"%02d\", ", minutes);
		fprintf(fpWH1080,"\"seconds\" : \"%02d\", ", seconds);
		fprintf(fpWH1080,"\"year\" : \"%02d\", ", year);
		fprintf(fpWH1080,"\"month\" : \"%02d\", ", month);
		fprintf(fpWH1080,"\"day\" : \"%02d\"}", day);		
		}	
fclose(fpWH1080);
} else {	
	FILE *fpWH3080;
	fpWH3080=fopen("/dev/shm/wh3080-rtl_433.txt", "w");
	if (msg_type == 2) {
		fprintf(fpWH3080,"{\"time\" : \"%s\", ", time_str);
		fprintf(fpWH3080,"\"msg_type\" : %01X, ", msg_type);
		fprintf(fpWH3080,"\"uv_sensor_id\" : \"%d\", ", uv_sensor_id);
		fprintf(fpWH3080,"\"uv_status\" : \"%s\", ", uv_status);
		fprintf(fpWH3080,"\"uv_index\" : %d, ", uv_index);
		fprintf(fpWH3080,"\"lux\" : %.1f, ", lux);
		fprintf(fpWH3080,"\"wm\" : %.2f, ", wm);
		fprintf(fpWH3080,"\"fc\" : %.2f}", fc);
	}
fclose(fpWH3080);
}
...
r_device fineoffset_wh1080_wh3080 = {
    ...
    .long_limit     = 2800,
    .reset_limit    = 2800,
    .json_callback  = &fineoffset_wh1080_wh3080_callback,
    .disabled       = 0,
    .demod_arg      = 0,
