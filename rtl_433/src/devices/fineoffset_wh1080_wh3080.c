...
if (msg_type == 0) {
    ...
    data = data_make(
			...
			"station_id",		"Station ID",	DATA_FORMAT,	"%d",		DATA_INT,	device_id,
			"temperature","Temperature",	DATA_FORMAT,	"%.01f C",	DATA_DOUBLE,	temperature,
			"humidity",	"Humidity",	DATA_FORMAT,	"%u %%",	DATA_INT,	humidity,
			"wind_dir_str","Wind string",	DATA_STRING,					direction_str,
			"wind_dir_deg","Wind degrees",	DATA_STRING,					direction_deg,
			"wind_speed",	"Wind avg speed",DATA_FORMAT,	"%.02f",	DATA_DOUBLE,	speed,
			"wind_gust",		"Wind gust",	DATA_FORMAT,	"%.02f",	DATA_DOUBLE, 	gust,
			"total_rain",		"Total rainfall",DATA_FORMAT,	"%3.1f",	DATA_DOUBLE, 	rain,
			"battery",	"Battery",	DATA_STRING,					battery,
		NULL);
	FILE *fpWH1080;
	fpWH1080=fopen("/dev/shm/wh1080-rtl_433.txt", "w");	
	data_print(data, fpWH1080, &data_json_printer, NULL); fprintf(fpWH1080, "\n");
	fclose(fpWH1080);
	...
	data_print(data, fpWH3080, &data_json_printer, NULL); fprintf(fpWH3080, "\n");
	fclose(fpWH3080);
    data_acquired_handler(data);
    return 1; 
    }
